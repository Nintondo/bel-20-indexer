...
pub trait RocksDbTablesDef: Sized {
    ...
    fn open(path: &str) -> anyhow::Result<Self> {
        let db = RocksDB::open_db(path, [&[internal::TABLE_INFO_CF, internal::DB_INFO_CF], Self::TABLES].into_iter().flatten());
        ...
        if let Some(db_info) = db_info.get(()) {
            if db_info.version > Self::VERSION {
                bail!("Version of DB '{}' is not supported: {}", std::any::type_name::<Self>(), db_info.version);
            }
            if db_info.version < Self::VERSION {
                tracing::warn!(
        ...
        db_info.set((), DbInfo { version: Self::VERSION });
