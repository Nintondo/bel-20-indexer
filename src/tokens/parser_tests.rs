...
fn try_parse_rejects_wrong_protocol() {
    ...
    assert!(matches!(BlockTokenState::try_parse(ct(), content, coin()), Err(Brc4ParseErr::WrongProtocol)));
}

#[test]
fn try_parse_rejects_invalid_utf8() {
    let content = b"\xFF\xFF";
    assert!(matches!(BlockTokenState::try_parse(ct(), content, coin()), Err(Brc4ParseErr::InvalidUtf8)));
}

#[test]
fn try_parse_accepts_valid_deploy() {
    let content = br#"{"p":"brc-20","op":"deploy","tick":"abcd","max":"100","lim":"100","dec":"0"}"#;
    let parsed = BlockTokenState::try_parse(ct(), content, coin()).unwrap();
    match parsed {
        Brc4::Deploy { proto } => {
...
fn try_parse_accepts_valid_mint_and_transfer() {
    ...
    assert!(matches!(BlockTokenState::try_parse(ct(), mint, coin()), Ok(Brc4::Mint { .. })));

    let transfer = br#"{"p":"brc-20","op":"transfer","tick":"abcd","amt":"1"}"#;
    assert!(matches!(BlockTokenState::try_parse(ct(), transfer, coin()), Ok(Brc4::Transfer { .. })));
}
