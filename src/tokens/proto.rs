...
impl MintProto {
    pub fn value(&self) -> anyhow::Result<MintProtoWrapper> {
        match self {
            ...
            MintProto::Junk20(v) if *BLOCKCHAIN == Blockchain::Junkcoin => Ok(*v),
...
impl DeployProto {
    pub fn value(&self) -> anyhow::Result<DeployProtoWrapper> {
        match self {
            ...
            DeployProto::Junk20(v) if *BLOCKCHAIN == Blockchain::Junkcoin => Ok(*v),
            _ => anyhow::bail!("Unsupported type"),
        }
    }
}
...
impl TransferProto {
    pub fn value(&self) -> anyhow::Result<MintProtoWrapper> {
        match self {
            ...
            TransferProto::Junk20(v) if *BLOCKCHAIN == Blockchain::Junkcoin => Ok(*v),
            _ => anyhow::bail!("Unsupported type"),
        }
    }
}
