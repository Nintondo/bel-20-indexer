...
pub async fn tokens(
    ...
    let iter = server
        ...
        .filter(|x| match args.filter_by {
            rest_api::TokenFilterBy::All => true,
            rest_api::TokenFilterBy::Completed => x.1.is_completed(),
            rest_api::TokenFilterBy::InProgress => !x.1.is_completed(),
        })
    ...
    let all = match args.sort_by {
        ...
        rest_api::TokenSortBy::DeployTimeDesc => iter
            .sorted_by_key(|(_, v)| v.proto.created)
            .rev()
            .collect_vec(),
        rest_api::TokenSortBy::HoldersAsc => iter
            .sorted_by_key(|(_, proto)| stats.get(&proto.proto.tick))
            .collect_vec(),
        rest_api::TokenSortBy::HoldersDesc => iter
            .sorted_by_key(|(_, proto)| stats.get(&proto.proto.tick))
            .rev()
            .collect_vec(),
        rest_api::TokenSortBy::TransactionsAsc => iter
            .sorted_by_key(|(_, v)| v.proto.transactions)
            .collect_vec(),
        rest_api::TokenSortBy::TransactionsDesc => iter
            .sorted_by_key(|(_, v)| v.proto.transactions)
            .rev()
            .collect_vec(),
    };
...
pub async fn token_transfer_proof(
    ...
    let data: Vec<_> = state
        ...
        .map(|(_, TransferProtoDB { tick, amt, height })| {
            anyhow::Ok(rest_api::TokenTransferProof { amt, tick, height })
        })
        .try_collect()
        .track_with("")
        .internal(INTERNAL)?;
