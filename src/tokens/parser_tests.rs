...
fn try_parse_rejects_wrong_protocol() {
    ...
    assert!(matches!(TokenCache::try_parse(ct(), content, 0, coin()), Err(Brc4ParseErr::WrongProtocol)));
}

#[test]
fn try_parse_rejects_invalid_utf8() {
    let content = b"\xFF\xFF";
    assert!(matches!(TokenCache::try_parse(ct(), content, 0, coin()), Err(Brc4ParseErr::InvalidUtf8)));
}

#[test]
fn try_parse_accepts_valid_deploy() {
    let content = br#"{"p":"brc-20","op":"deploy","tick":"abcd","max":"100","lim":"100","dec":"0"}"#;
    let parsed = TokenCache::try_parse(ct(), content, 0, coin()).unwrap();
    match parsed {
        Brc4::Deploy { proto } => {
...
fn try_parse_accepts_self_mint_max_zero_lim_omitted() {
    ...
    let parsed = TokenCache::try_parse(ct(), content, coin().self_mint_activation_height.unwrap_or_default() as u32, coin()).unwrap();
    match parsed {
        Brc4::Deploy { proto } => {
...
fn try_parse_accepts_valid_mint_and_transfer() {
    ...
    assert!(matches!(TokenCache::try_parse(ct(), mint, 0, coin()), Ok(Brc4::Mint { .. })));

    let transfer = br#"{"p":"brc-20","op":"transfer","tick":"abcd","amt":"1"}"#;
    assert!(matches!(TokenCache::try_parse(ct(), transfer, 0, coin()), Ok(Brc4::Transfer { .. })));
}
